...
<body>
		...
		<header>
			<div class="container-fluid">
				<div class="header d-md-flex justify-content-between align-items-center py-3 px-xl-5 px-lg-3 px-2">
					<!-- logo -->
					<div id="logo">
						<h1><a href="../home.php.html">MERCEDES BENZ</a><img src="../symbol3 - Copy.png" style="position:relative;margin-left:10px;top:-18px; width:40px;"/></h1>
					</div>
					<!-- //logo -->
					<!-- nav -->
					<div class="nav_w3ls">
	...
	<div class="breadcrumb-mobamu">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">MECEDES TOP-UP BOOKING</li>
		</ol>
	</div>
	<!-- //page details -->
	...
	<div class="register-w3 py-5">
		<div class="container py-xl-5 py-lg-3">
			<h3 class="tittle text-center text-bl font-weight-bold">MECEDES TOP-UP BOOKING</h3>
			<div class="row">
...
<?php
	...
	$bookingid=1;
	$res=$obj->execute("select max(TPID)+1 as tpid from topup");
...
?>
							...
							<div class="col-lg-6 form-group">
								...
								<input class="form-control" type="text" name="uname"  value="<?php echo $_SESSION["email"]; ?>" readonly="true"  />
								
							</div>
							
					
							<div class="col-lg-6 form-group">
								<label>Top-Up ID</label>
								<input class="form-control" type="text" name="tid"   value="<?php echo $bookingid;?>" readonly="true" />
								
							</div>
							<div class="col-lg-6 form-group">
								<label>Location</label>
								<input class="form-control" type="text" name="loc"  value="<?php echo $_SESSION["location"];?>" readonly="true"">
								
							</div>
							<div class="col-lg-6 form-group">
								<label>Pin</label>
								<input class="form-control" type="text" name="pin" value="<?php echo $_SESSION["pin"];?>" readonly="true"">
								
							</div>
								<div class="col-lg-6 form-group">
								<label>House Nmae</label>
								<input class="form-control" type="text" name="hname" value="<?php echo $_SESSION["hname"];?>" readonly="true"" >
							</div>
							
							<div class="col-lg-6 form-group">
								<label>Time</label>
								<input class="form-control" type="time" name="ttime"  placeholder="Time">
			...
			$cdate="";
			$res=$obj->execute("select CURRENT_DATE");
			if($row=mysqli_fetch_array($res))
			{
				$cdate=$row[0];
			}
			...
			?>
								...
								<input class="form-control" min=<?php echo $cdate; ?> type="date" name="tdate" >
							</div>
						<div class="col-lg-6 form-group">
								<label>Mobile Number</label>
								<input class="form-control" type="text" name="mob"  value="<?php echo $_SESSION["mobile"];?>" readonly="true"/>
								
							</div>							<div class="col-lg-6 form-group">
								<label>Complaint</label>
								<textarea class="form-control" type="text" name="cmp"  placeholder="Complaint" >
								</textarea>
							</div>
					...
					<input type="submit" value="SUBMIT FORM" name="btnsubmit" onClick="return valid()"class="btn submit mt-3">
					</form>
				</div>
			</div>
		</div>
	...
	if(isset($_POST["btnsubmit"]))
	{
		...
		$obj->execute("insert into topup values(null, '$model','$uname','$ttime', '$tdate', '$cmp')");
		...
		$res=$obj->execute("select max(tpid) as tpid from topup where uname='". $_SESSION["email"] ."'");
	if($row=mysqli_fetch_array($res))
	{
		$_SESSION["tpid"]=$row["tpid"];
	}	
	 
		$msg="Your service and complaint for the vehicle '$mname'  will be processed on '$tdate' at '$ttime'";
		SendEmail($msg);
		//echo $obj->msg("Complaint Booking Completed Successfully");
		$obj->redirect("bill/bill.php");
		//$obj->redirect("../viewcomplaintbooking.php");
	}
